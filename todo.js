...
const templateItem = document.querySelector(".listHere"); // Make sure this matches your actual HTML structure
...
async function updateToDo(id, payload) {
  try {
    const response = await fetch(`${apiUrl}/todos/${id}`, { // Corrected endpoint path with backticks
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
...
async function getTodos() {
  try {
    ...
    if (!response.ok) {
      throw new Error("Didn't get information from the Database ..........");
    }
